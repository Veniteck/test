...
if(! function_exists('vg_calaco_product_carousel_widget'))
{
	function vg_calaco_product_carousel_widget() {
		register_widget('Vina_ProductCarousel_Widget');
	}
}
add_action('widgets_init', 'vg_calaco_product_carousel_widget');


// get product carousel theme
if(! function_exists('vg_calaco_product_carousel_themes'))
{
	function vg_calaco_product_carousel_themes($active, $type = "list") {
		$themes = ($type == "list") ? "" : array();
		$path 	= get_template_directory() . "/woocommerce/product-carousel";
...
// function get WooCommerce categories
if(! function_exists('vg_calaco_get_categories_array'))
{
	function vg_calaco_get_categories_array($results = array(), $parent = 0) 
	{
		$cats = get_terms('product_cat' , array('hide_empty' => false, 'hierarchical' => true, 'parent' => $parent));
		...
		if(!empty($cats)) {
			foreach($cats as $cat) {
				...
				$results = vg_calaco_get_categories_array($results, $cat->term_id);
			}		
		}
...
if(! class_exists('Vina_ProductCarousel_Widget')) 
{
	class Vina_ProductCarousel_Widget extends WP_Widget 
	{	
		/**
		...
		{
			parent::__construct(
				'vg_calaco_product_carousel', // Base ID
				esc_html__('VGW Product Carousel', 'vg-calaco'), // Name
				array('description' => esc_html__('A widget that display products of WooCommerce in responsive carousel.', 'vg-calaco')) // Args
			);
		}
		...
		public function widget($args, $instance) 
		{
			...
			if($total_items && is_file(get_template_directory() . '/woocommerce/product-carousel/' . $carousel_theme)) {
				include get_template_directory() . '/woocommerce/product-carousel/' . $carousel_theme;
			}
		...
		public function form($instance) 
		{
			...
			$carousel_type  = !empty($instance['carousel_type']) ? $instance['carousel_type'] : "latest";
			$categories 	= vg_calaco_get_categories_list(0, $cActive);
			$order_by		= !empty($instance['order_by']) ? $instance['order_by'] : "date";
			$order			= !empty($instance['order']) ? $instance['order'] : "desc";
			$carousel_theme = !empty($instance['carousel_theme']) ? $instance['carousel_theme'] : "default.php";
			...
			<p>
				...
				<em><?php _e('Enter unique ID for this carousel. Eg: vgw-product-carousel, vgw-product-carousel-1, vgw-product-carousel-2 ...', 'vg-calaco'); ?></em>
			</p>
			<p>
			...
			<p>
				...
				<select class="widefat" name="<?php echo esc_attr($this->get_field_name('carousel_type')); ?>" id="<?php echo esc_attr($this->get_field_id('carousel_type')); ?>">				
					<option value="all"<?php echo($carousel_type == 'all') ? ' selected="selected"' : ""; ?>><?php esc_html_e('All Products', 'vg-calaco'); ?></option>
					<option value="featured"<?php echo($carousel_type == 'featured') ? ' selected="selected"' : ""; ?>><?php esc_html_e('Featured Products', 'vg-calaco'); ?></option>
					<option value="onsale"<?php echo($carousel_type == 'onsale') ? ' selected="selected"' : ""; ?>><?php esc_html_e('On-sale Products', 'vg-calaco'); ?></option>
				</select>
			</p>
			<p>
			...
			<p>
				<label for="<?php echo esc_attr($this->get_field_id('order_by')); ?>"><?php esc_html_e('Order by:', 'vg-calaco'); ?></label>
				<select class="widefat" name="<?php echo esc_attr($this->get_field_name('order_by')); ?>" id="<?php echo esc_attr($this->get_field_id('order_by')); ?>">				
					<option value="date"<?php echo($order_by == 'date') ? ' selected="selected"' : ""; ?>><?php esc_html_e('Date', 'vg-calaco'); ?></option>
					<option value="price"<?php echo($order_by == 'price') ? ' selected="selected"' : ""; ?>><?php esc_html_e('Price', 'vg-calaco'); ?></option>
					<option value="rand"<?php echo($order_by == 'rand') ? ' selected="selected"' : ""; ?>><?php esc_html_e('Random', 'vg-calaco'); ?></option>
					<option value="sales"<?php echo($order_by == 'sales') ? ' selected="selected"' : ""; ?>><?php esc_html_e('Sales', 'vg-calaco'); ?></option>								
				</select>
			</p>
			<p>
				<label for="<?php echo esc_attr($this->get_field_id('order')); ?>"><?php esc_html_e('Order:', 'vg-calaco'); ?></label>
				<select class="widefat" name="<?php echo esc_attr($this->get_field_name('order')); ?>" id="<?php echo esc_attr($this->get_field_id('order')); ?>">				
					<option value="desc"<?php echo($order == 'desc') ? ' selected="selected"' : ""; ?>><?php esc_html_e('DESC', 'vg-calaco'); ?></option>
					<option value="asc"<?php echo($order == 'asc') ? ' selected="selected"' : ""; ?>><?php esc_html_e('ASC', 'vg-calaco'); ?></option>
				</select>
			</p>
			<p>
				...
				<select class="widefat" name="<?php echo esc_attr($this->get_field_name('carousel_theme')); ?>" id="<?php echo esc_attr($this->get_field_id('carousel_theme')); ?>">				
					<?php echo vg_calaco_product_carousel_themes($carousel_theme); ?>
				</select>
			</p>
			<p>
				<label for="<?php echo esc_attr($this->get_field_id('total_product')); ?>"><?php esc_html_e('Total Products:', 'vg-calaco'); ?></label> 
		...
		public function update($new_instance, $old_instance) 
		{
			...
			$instance['unique_id'] 		= (!empty($new_instance['unique_id'])) ? strip_tags($new_instance['unique_id']) : 'vgw-product-carousel';
		 ...
		 * [vgw_product_carousel class_suffix="WIDGET_CLASS_SUFFIX" ...]
		 */
		 
		public static function shortcode($atts, $content = "")
		{
			$widget = new Vina_ProductCarousel_Widget;
...
if(! class_exists('VinaProductCarouselAddonClass')) 
{
	class VinaProductCarouselAddonClass 
	{
		...
		public function integrateWithVC() {
			...
			*/
			...
			vc_map(array(
				"name" 			=> esc_html__("VGW Carousel", 'vg-calaco'),
				"description" 	=> esc_html__("Product carousel for WooCommerce.", 'vg-calaco'),
				"base" 			=> "vgw_product_carousel",
				"class" 		=> "",
				"controls" 		=> "full",
				...
				"params" => array(
					array(
						...
						"value" 		=> "vgw-product-carousel",
						"admin_label" 	=> true,
						"description" 	=> esc_html__('Enter unique ID for this carousel. Eg: vgw-product-carousel, vgw-product-carousel-1, vgw-product-carousel-2 ...', 'vg-calaco'),
					),
					array(
						"type" 			=> "textfield",
						"save_always" 	=> true,
					...
					array(
						...
						"value" 		=> array(
							esc_html__('All Products', 'vg-calaco') 		=> 'all',
							esc_html__('Featured Products', 'vg-calaco') 	=> 'featured',
							esc_html__('On-sale Products', 'vg-calaco') 	=> 'onsale',							
						),
						"description" 	=> "",
						"admin_label" 	=> true,
					),
					array(
						...
						"value" 		=> vg_calaco_get_categories_array(),
						"description" 	=> "",
					...
					array(
						...
						"save_always" 	=> true,
						"class" 		=> "",
						"heading" 		=> esc_html__("Order by", 'vg-calaco'),
						"param_name" 	=> "order_by",
						"value" 		=> array(
							esc_html__('Date', 'vg-calaco') 	=> 'date',
							esc_html__('Price', 'vg-calaco') 	=> 'price',
							esc_html__('Random', 'vg-calaco') 	=> 'rand',
							esc_html__('Sales', 'vg-calaco') 	=> 'sales',
						),
						"description" 	=> ""
					),
					array(
						...
						"save_always" 	=> true,
						"class" 		=> "",
						"heading" 		=> esc_html__("Order", 'vg-calaco'),
						"param_name" 	=> "order",
						"value" 		=> array(
							esc_html__('DESC', 'vg-calaco') 	=> 'desc',
							esc_html__('ASC', 'vg-calaco') 		=> 'asc',
						),
						"description" 	=> ""
					),
					array(
						...
						"value" 		=> vg_calaco_product_carousel_themes("", "array"),
						"description" 	=> "",
						"admin_label" 	=> true,
					),
					array(
						...
						"save_always" 	=> true,
						"class" 		=> "",
						"heading" 		=> esc_html__("Total Products", 'vg-calaco'),
...
new VinaProductCarouselAddonClass();