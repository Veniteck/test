<?php 
function vg_calaco_breadcrumbs() {
	$showOnHome = 0;
	$showCurrent = 1;
	$delimiter = '<span class="divide"> <i class="fa fa-angle-right" aria-hidden="true"></i> </span>';
	...
	if (is_home() || is_front_page()) {
		...
		echo '<div id="crumbs">';
			echo '<a href="' . esc_url($homeLink) . '">' . $home . '</a> ' . $delimiter . ' ';
				...
				echo ($before) .  esc_html__('Articles posted by ', 'vg-calaco') . esc_html($userdata->display_name) . ($after);

			} elseif ( is_404() ) {
				echo ($before) . esc_html__('Error 404', 'vg-calaco') . ($after);
			}

			if ( get_query_var('paged') ) {
				if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo ' (';
				echo esc_html__('Page', 'vg-calaco') . ' ' . get_query_var('paged');
				if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo ')';
			}
		echo '</div>';

	}
} // end vg_calaco_breadcrumbs()
