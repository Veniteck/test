...
if(! function_exists('vg_calaco_woo_testimonial_widget'))
{
	function vg_calaco_woo_testimonial_widget() {
		register_widget('Vina_WooTestimonial_Widget');
	}
}
add_action('widgets_init', 'vg_calaco_woo_testimonial_widget');


// get product carousel theme
if(! function_exists('vg_calaco_woo_testimonial_themes'))
{
	function vg_calaco_woo_testimonial_themes($active, $type = "list") {
		$themes = ($type == "list") ? "" : array();
		$path 	= get_template_directory() . "/includes/testimonial/template";
...
if(! class_exists('Vina_WooTestimonial_Widget')) 
{
	class Vina_WooTestimonial_Widget extends WP_Widget 
	{	
		/**
		...
		{
			parent::__construct(
				'vg_calaco_woo_testimonial', // Base ID
				esc_html__('VGW Woo Testimonial', 'vg-calaco'), // Name
		...
		public function widget($args, $instance) 
		{
			...
			if($total_items && is_file(get_template_directory() . '/includes/testimonial/template/' . $carousel_theme)) {
				include get_template_directory() . '/includes/testimonial/template/' . $carousel_theme;
			}
		...
		public function form($instance) 
		{
			...
			$unique_id 		= !empty($instance['unique_id']) ? $instance['unique_id'] : "vgw-woo-testimonial";
			...
			$items_visible  = !empty($instance['items_visible']) ? $instance['items_visible'] : "1";
			$row_carousel	= !empty($instance['row_carousel']) ? $instance['row_carousel'] : "1";
			
			$responsive		= !empty($instance['responsive']) ? $instance['responsive'] : "";
			$items_desktop	= !empty($instance['items_desktop']) ? $instance['items_desktop'] : "[1199,1]";
			$items_sdesktop	= !empty($instance['items_sdesktop']) ? $instance['items_sdesktop'] : "[979,1]";
			$items_tablet	= !empty($instance['items_tablet']) ? $instance['items_tablet'] : "[768,1]";
			...
			<p>
				...
				<em><?php _e('Enter unique ID for this carousel. Eg: vgw-woo-testimonial, vgw-woo-testimonial-1, vgw-woo-testimonial-2 ...', 'vg-calaco'); ?></em>
			</p>
			<p>
			...
			<p>
				...
				<select class="widefat" name="<?php echo esc_attr($this->get_field_name('carousel_theme')); ?>" id="<?php echo esc_attr($this->get_field_id('carousel_theme')); ?>">				
					<?php echo vg_calaco_woo_testimonial_themes($carousel_theme); ?>
				</select>
			</p>
			<p>
		...
		public function update($new_instance, $old_instance) 
		{
			...
			$instance['unique_id'] 		= (!empty($new_instance['unique_id'])) ? strip_tags($new_instance['unique_id']) : 'vgw-woo-testimonial';
			...
			$instance['items_visible'] 	= (!empty($new_instance['items_visible'])) ? intval($new_instance['items_visible']) : '1';
			...
			$instance['items_desktop']	= !empty($new_instance['items_desktop']) ? $new_instance['items_desktop'] : "[1199,1]";
			$instance['items_sdesktop']	= !empty($new_instance['items_sdesktop']) ? $new_instance['items_sdesktop'] : "[979,1]";
			$instance['items_tablet']	= !empty($new_instance['items_tablet']) ? $new_instance['items_tablet'] : "[768,1]";
		 ...
		 * [vgw_woo_testimonial class_suffix="WIDGET_CLASS_SUFFIX" ...]
		 */
		 
		public static function shortcode($atts, $content = "")
		{
			$widget = new Vina_WooTestimonial_Widget;
...
if(! class_exists('VinaWooTestimonialAddonClass')) 
{
	class VinaWooTestimonialAddonClass 
	{
		...
		public function integrateWithVC() {
			...
			*/
			...
			vc_map(array(
				"name" 			=> esc_html__("VGW Woo Testimonial", 'vg-calaco'),
				"description" 	=> esc_html__("A responsive carousel for WordPress.", 'vg-calaco'),
				"base" 			=> "vgw_woo_testimonial",
				"class" 		=> "",
				"controls" 		=> "full",
				"icon" 			=> "icon-wpb-wp",
				...
				"params" => array(
					array(
						...
						"value" 		=> "vgw-woo-testimonial",
						"admin_label" 	=> true,
						"description" 	=> esc_html__('Enter unique ID for this carousel. Eg: vgw-woo-testimonial, vgw-woo-testimonial-1, vgw-woo-testimonial-2 ...', 'vg-calaco'),
					),
					array(
						"type" 			=> "textfield",
						"save_always" 	=> true,
					...
					array(
						...
						"value" 		=> vg_calaco_woo_testimonial_themes("", "array"),
						"description" 	=> "",
						"admin_label" 	=> true,
					),
					array(
					...
					array(
						...
						"value" 		=> "[1199,1]",
						"description" 	=> esc_html__("This allows you to preset the number of slides visible with a particular browser width. The format is [x,y] whereby x=browser width and y=number of slides displayed. For example [1199,1] means that if(window<=1199){ show 1 slides per page}", 'vg-calaco'),
						'group' 		=> esc_html__('Responsive', 'vg-calaco'),
					),
					array(
						"type" 			=> "textfield",
		...
		*/
		public function showVcVersionNotice() {			
			echo '
			<div class="updated">
				<p>'.sprintf(__('<strong>%s</strong> requires <strong>Visual Composer</strong> plugin to be installed and activated on your site.', 'vg-calaco'), "VGW Woo Testimonial").'</p>
			</div>';
		}
	}
}
new VinaWooTestimonialAddonClass();