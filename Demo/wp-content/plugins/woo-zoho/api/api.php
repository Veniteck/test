...
$custom=array('invoices','contacts','estimates','purchaseorders','salesorders'); 
...
if(in_array($module,array('Sales_Orders','Purchase_Orders','Invoices'))){
 foreach($zoho_products as $v){ 
     ...
     if( $v['price'] > $v['cost']){ 
      //   $item_arr['Discount']= $v['price']-$v['cost'];
...
public function push_object_invoice($module,$fields,$meta){
    ...
    /* $res=$this->post_crm('invoices/2042359000000074014');
    var_dump($res); die();
...
if(!empty($related['contacts']) ){
 ...
 //change owner id
  if(isset($meta['owner']) && $meta['owner'] == "1"){
   ...
   $post['customer_id']=self::$feeds_res[$meta['object_contact']]['id'];   
   $fields['customer_id']=array('label'=>'Customer ID','value'=>$post['customer_id']);
  } 
 ...
 if(is_array($zoho_products) && count($zoho_products)>0){
 foreach($zoho_products as $v){
$post['line_items'][]=array('item_id'=>$v['id'],'quantity'=>$v['qty']);   
}   
...
if(!empty($method)){
if(empty($path)){  $path=$module; }
if(!empty($meta['email_check'])){ $path.='?send=true'; }
...
if(!empty($id)){
   ...
   $module_url= $module == 'customerpayments' ? 'paymentsreceived' : str_replace('_','',$module);
...
$sku=$img_id=''; $qty=$unit_price=$tax=$total=$cost=$stock=0;
if(method_exists($item,'get_product')){
  ...
  // $title=$item->get_name();
   ...
   $unit_price=$product->get_price();   
    ...
    if(method_exists($_order,'get_item_total')){
       $cost=$_order->get_item_total($item,false,true);
    }
  ...
  $temp=array('sku'=>$sku,'unit_price'=>$unit_price,'title'=>$title,'qty'=>$qty,'tax'=>$tax,'total'=>$total,'cost'=>$cost,'qty_stock'=>$stock,'img_id'=>$img_id);
          if(method_exists($product,'get_stock_quantity')){
...
public function get_zoho_products($meta){ 
 ...
 $product_detail=array('price'=>$unit_price,'qty'=>$qty,'tax'=>$tax,'total'=>$total,'cost'=>$cost);
...
if($this->info['type'] == 'books'){ //books support sku search but sku is not enabled by default in books
