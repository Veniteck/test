...
public function save_product($post_id,$post){
   if(isset($post->post_status) && $post->post_status != 'trash' && !defined( 'DOING_CRON' ) ){ 
...
if( $status == '' && in_array($event,$other_events)){ // skip user and product on manual submission
    continue;
}
  $feed['id']=$id; 
...
//echo 'feed found ------';
       ...
       }else if($moved[$id] < 5 ){ // only 5 moves per feed
...
if(strpos($f_key,"__vx_wp-") ===0){ //if user field
  ...
  if(method_exists(self::$_order,'get_user_id')){
 ...
 $value=$user->$f_key;    
  ...
  $value=get_user_meta($user_id,$f_key,true);
      ...
      if($f_key_type == '__vxp_fun-'){
          $fun='get_'.$f_key; 
         $value=$product->$fun();
         if( in_array($f_key,array('short_description','description'))){
