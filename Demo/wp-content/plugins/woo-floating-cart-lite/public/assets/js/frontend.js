	...
	*/
	...
	$(function() {
		...
		function initVars() {
			...
			cartList = cartBody.find('ul.xt_woofc-list');
		...
		function findSingleImage(trigger) {
			...
			// Find image in Woo Quick View Modal
				...
				if($('body').hasClass('xt_wooqv-fullscreen')) {
					item = $('.xt_wooqv-slider-wrapper .xt_wooqv-slider li');
				}else{
		...
		window.xt_woofc_refresh_fragments = refreshFragments;
	});
